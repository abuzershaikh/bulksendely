...
function Core(){
    ...
    this.actionItem= function(){
        $(document).on('click', ".actionItem", function(event) {
            ...
            // Omnichannel channel tab handoff via sessionStorage
            var ch = that.data("channel");
            if (ch && action && action.indexOf('/omnichannel') !== -1) {
                sessionStorage.setItem('omni_channel', ch);
            }
    ...
    this.ajax_post = function(that, action, data, _function){
        ...
        if(history_url != undefined){
            ...
            if(!that.hasClass("disabled")){
                ...
                // Lock submit button to prevent double clicks and show loading state
                var btn = that;
                ...
                if(btn.length > 0){
                    ...
                    // Only add spinner if there isn't one already
                    if(btn.find('.fa-spinner').length === 0){
                        btn.data('original-text', btnText);
                        btn.html('<i class="fas fa-spinner fa-spin"></i> ' + btnText);
                    }
                }
                ...
                $.post(action, data, function(result){
                    if(result === undefined || result === null){
                        ...
                        if(btn.length > 0){
                            btn.removeClass("disabled").prop("disabled", false).html(btn.data('original-text') || btnText);
                        }
                        self.notify("Error: Unxpected empty response", "error");
                        return;
                    }
            ...
            }, type).fail(function(xhr, status, error) {
                //Hide Loading
                self.overplay(true);
                that.removeClass("disabled");
                if(btn.length > 0){
                ...
                var actStr = (typeof action !== 'undefined' ? action : 'unknown');
                self.notify("Error: " + error + " (" + xhr.status + ") - URL: " + actStr.substring(0, 100), "error");
            });
        }
        ...
        if ( that.length > 0 )
        {
            ...
            $("."+scrollDiv).on('scroll',function(){
                ...
                scrollTimeout = setTimeout(function() {
                    ...
                    var _container = $("."+scrollDiv);
                    var _scrollTop = _container.scrollTop();
                    var _divHeight = _container.height();
                    var _scrollHeight = _container.get(0).scrollHeight;

                    // Removed $(window).trigger('resize') - Major performance killer
    ...
    this.ajax_load_scroll = function( reset_page, scroll_no){
        ...
        var that = $('.ajax-load-scroll'+index+':visible');
        if (that.length == 0) {
            that = $('.ajax-load-scroll'+index).first();
        }
        var url = that.attr('data-url');
        
        var filter = that.closest('.fm-widget').find(".ajax-filter"+index);
        if (filter.length == 0) filter = that.parent().find(".ajax-filter"+index);
        if (filter.length == 0) filter = $(".ajax-filter"+index+':visible');
        ...
        if ( that.length > 0 ){
            ...
            if ( loading == undefined || loading == 0 )
            {
                ...
                $.ajax({
                    ...
                    $('.ajax-loading').hide();
                    console.log('ajax_load_scroll success. Page:', page, 'Result size:', result.length);
                    ...
                    that.attr('data-loading', 0); 
                    $(document).trigger('core_ajax_load_scroll_done', [that]);
                    ...
                    $('.ajax-loading').hide();
                    console.error('ajax_load_scroll failed', textStatus, error);
                    that.attr('data-loading', 0);
                });
            }
        }
    ...
    this.select2 = function(){
        try {
            ...
            // Silently fail if select2 is not available
