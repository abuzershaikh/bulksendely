...
class Whatsapp_contact extends \CodeIgniter\Controller
{
    ...
    public function exportar_csv($ids = false)
    {
        ...
        $builder->select('phone, params, params')
                ->where('pid', $pid->id);

        $query = $builder->get();
        $results = $query->getResult();

        $cabecalho = ['Phone', 'Name', 'Params'];
        ...
        foreach ($results as $row) {
            ...
            $name = isset($params['Nome']) ? $params['Nome'] : '';
            fputcsv($output, [$row->phone, $name, $name], ',');
        }
