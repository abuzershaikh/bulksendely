...
class Profile extends \CodeIgniter\Controller
{
    ...
    public function save_account(){
        ...
        if (get_option("accept_change_whatsapp", 1)) {
            validate("null", __("WhatsApp"), $whatsapp);
            validate("whatsapp", __("WhatsApp"), $whatsapp);
            validate("min_length", __("WhatsApp"), $whatsapp, 11);
            $check_whatsapp = db_get("id", TB_USERS, ["whatsapp" => $whatsapp, "id != " => $user_id]);
            validate("not_empty", __("The whatsapp already exists."), $check_whatsapp);
        }
    ...
    public function download_invoice($invoice_id = ""){
        ...
        $fontDirs[] = get_module_dir(__DIR__, "Assets/");
