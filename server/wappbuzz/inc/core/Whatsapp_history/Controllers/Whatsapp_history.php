...
namespace Core\Whatsapp_history\Controllers;

class Whatsapp_history extends \CodeIgniter\Controller
{
    public function __construct()
    {
        $this->config = parse_config(include realpath(__DIR__ . "/../Config.php"));
        $this->model = new \Core\Whatsapp_history\Models\Whatsapp_historyModel();
    }
    ...
    public function index($page = false, $ids = false)
    {
        ...
        switch ($page) {
            case 'update':
                ...
                $item = false;
                if ($ids) {
                    $item = db_get("*", TB_WHATSAPP_HISTORY, ["ids" => $ids, "team_id" => $team_id]);
                }

                $contacts = db_fetch("*", TB_WHATSAPP_CONTACTS, ["team_id" => $team_id, "status" => 1], "id", "DESC");

                $data['content'] = view('Core\Whatsapp_history\Views\update', ["result" => $item, "contacts" => $contacts, "config" => $this->config]);
                break;
            ...
            default:
                ...
                $data['content'] = view('Core\Whatsapp_history\Views\content', $data_content);
                break;
        }
    ...
    public function ajax_list()
    {
        ...
        ms([
            ...
            "data" => view('Core\Whatsapp_history\Views\ajax_list', $data)
        ]);
    }
    ...
    public function report($ids = "")
    {
        ...
        $report = view('Core\Whatsapp_history\Views\report', ['result' => $result]);
    ...
    public function report_by_day()
    {
        ...
        $report = view('Core\Whatsapp_history\Views\report_by_day', ['result' => $result]);
        header("Content-type: application/vnd.ms-excel");
        header("Content-Disposition: attachment; filename=$file");
        echo $report;
    }
    ...
    public function popup_report()
    {
        ...
        return view('Core\Whatsapp_history\Views\popup_report', $data);
    }
    ...
    public function delete($ids = false)
    {
        ...
        db_delete(TB_WHATSAPP_HISTORY, ['ids' => $ids, "team_id" => $team_id]);
