<?php
namespace Core\Whatsapp_history\Models;
use CodeIgniter\Model;

class Whatsapp_historyModel extends Model
{
    ...
    public function get_list( $return_data = true )
    {
        ...
        $builder = $db->table(TB_WHATSAPP_HISTORY);
        $builder->select('*');
    ...
    public function get_report_by_day(){
        ...
        $builder->join(TB_WHATSAPP_CONTACTS." as c", "a.contact_id = c.id");
        $builder->where("a.team_id", $team_id);
        $builder->where("a.changed BETWEEN '$date_since' AND '$date_until'");
        $query = $builder->get();
        $result = $query->getResult();
        return $result;
    }
