...
class AuthModel extends Model
{
    ...
    public function signup(){
		...
		// Only validate WhatsApp if OTP is enabled
		$otp_status = get_option('otp_status', 0);
		if($otp_status == 1) {
		...
		// Only require OTP verification if OTP is enabled
		if($otp_status == 1 && !isset($opt_verified)){
			ms([
				"status"  => "error",
        ...
        // Only check for duplicate WhatsApp if OTP is enabled and phone is provided
        if($otp_status == 1 && !empty($phone)) {
