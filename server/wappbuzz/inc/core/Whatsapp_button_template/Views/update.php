						...
						<div class="card-body">
					        ...
					        <div class="tab-content pt-3" id="nav-tabContent">
					            ...
					            <?php
                                    ...
                                    if (!function_exists('extract_code_from_url')) {
                                        // Função para extrair o código da URL
                                        function extract_code_from_url($url) {
                                            $code = '';
                                            if ($url) {
                                                $url_parts = parse_url($url);
                                                parse_str(isset($url_parts['query']) ? $url_parts['query'] : '', $params);
                                                $code = isset($params['code']) ? htmlspecialchars($params['code']) : '';
                                            }
                                            return $code;
                                        }
                                    }
                                    
                                    // Obtém o URL do valor
                                    $urlButton = get_data($value, "urlButton");
                                    $full_url = $urlButton != false ? get_data($urlButton, "url") : '';
                                    $extracted_code = extract_code_from_url($full_url);
                                ?>
